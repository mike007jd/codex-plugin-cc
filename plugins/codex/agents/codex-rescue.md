...
- Do not pass `--background` to `codex-companion`. The outer `/codex:rescue` command already chose whether the main Claude thread should wait on this subagent; the subagent itself should block on its single `Bash` call so the full Codex output comes back as the subagent's final message.
